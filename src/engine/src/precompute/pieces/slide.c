#include "precompute/pieces/slide.h"
#include "utils.h"
#include <alloca.h>
#include <semaphore.h>
#include <stdlib.h>
...
typedef Vector VectorVectorBitboard;
typedef Vector VectorByte;
void generate_bishop_bitboards() {
  ...
  VectorVectorBitboard data = vector_init(sizeof(struct vector), 64);

  // Vector to keep possible moves from specific square
  VectorByte moves = vector_init(sizeof(uint8_t), 64);
  ...
  for (uint8_t sq = 0; sq < 64; sq++) {
    ...
    for (int k = 0; k < 4; k++) {
      ...
      while (sq_moved >= 0 && sq_moved <= 63 && file_moved >= 0 &&
        ...
        vector_push(&moves, &sq_moved);
        sq_moved += dir[k];
        file_moved += file_delta[k];
      }
      if (good_dir) {
        attack_mask[sq] &= ~(1ULL << (sq_moved - dir[k]));
        vector_pop_back(&moves, NULL); // Last one is irelevant
      }
    }

    // Vector to keep occupancy + attack combinations
    VectorOccupancyAttack oa_vec =
        vector_init(sizeof(OccupancyAttack), 1ULL << moves.count);
